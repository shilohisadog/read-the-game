...
| **[what-settles.md](what-settles.md)** | ⭐⭐⭐ **THE EVIDENCE BEHIND THE CLUB/LEAGUE SPLIT, AND THE ARGUMENTS AGAINST IT.** Read before adding a card row and before answering a critic. All **47 candidates** measured over 4,192 games / 96 club-seasons, ranked by centred games-to-settle. ⛔⛔⛔ §1: `reliability.js` pooled seasons WITHOUT centring them against its own spec in `preview-and-corsi.md` §11.2 — visible for weeks as doc-37 vs code-42 on the slot row — which shipped `missed` (33 pooled, **113 centred**) and dropped `slot` (**38 centred**). Fixed 2026-09-23. ⛔ §3: Corsi / Fenwick / SF% / attempt differential are pairwise **0.84–0.98** — one measurement, not four; the card's three rows are already near-orthogonal (|r| ≤ 0.37). ⚠️ §4: the four venue instruments DISAGREE, so the judgement test may inform a decision and may NOT be a gate. §5: the four criticisms and which are CORRECT (chronological is conservative; 0.7 and 41 are declared policies; reliability ≠ usefulness) — and the reply that the bar excludes the standings (points % needs 55 games, save% 219). §6: what the feed cannot support, and ⚠️ per-game denominators as the strongest remaining attack. §7: `avgShiftLen` (12 games, orthogonal, survives every control) is the open recommendation. ⭐ **Its public face is `how-we-measure.html`**, built 2026-09-23: every figure the preview card draws doors into it, the arguments are conceded on the page, and `test/methods.test.js` fails the build if a figure has no derivation. |
| **[preview-page.md](preview-page.md)** | ✅ **BUILT AND LIVE 2026-09-23 (§10) — the page, its data and its doors; ⏭ the edge unfurl (§4) and og:image are NOT built.** Where the preview card lives: **`preview.html?game=<id>`**, one URL per game with three states (before / under way / played → replay door), so a shared link outlives the game. Kevin wants it shareable on X, so page and unfurl ship together — which makes `functions/_middleware.js` the **first server-side code here**, touching one path, passing through on error, computing no figure. ⭐ `teams.json` already holds the slot row; it gains a `through` date and two fields, and the page merges the nightly `recent.json` tail so figures are current to last night. ⚠️ Preseason shows three league rows and three *no games yet* rows, by the silence ruling. S1–S4 for CHENG. |
| **[front-door-tonight.md](front-door-tonight.md)** | ✅ **BUILT 2026-09-22** (§7; gates 1,358 JS + 224 Python). Kevin, from the live front door: *Next* named one game of eight. It is one by design, and §12.3 of `front-door.md` ruled it so — *a count over a window we cannot date honestly is a number with no population*. ⭐ The league already dates the night: `classify()` attaches `week.date` and the `upcoming` copy loop drops it. Carrying it (plus **one** more schedule request for the week ahead) gives the whole night an honest population, keeps *Tonight* earned like *Last night*, and fixes ⛔ the in-season defect: from 29 September `slate` carries no forward look at all. Tests listed with their mutations; ✅ all three decisions ruled by Kevin: rows as text, one line in season, +1 request. ✅ **CHENG endorsed it (§5) with four additions**: the degradation must announce itself (placed at the ingest pre-sync check, which REFUSES to publish, rather than in reader-facing ledger copy), the disclosure is closed on first paint and its summary carries the count, one test as a property (no `upcoming` row produces an href), and the Europe case is a correct oddity. Measured after: the tonight line costs **35 px**, and a 16-game night renders at the same 996 px as an 8-game one. |
