...
**Health right now:** 4,553 archived · **4,490 published** · 63 refused · 73 unreconciled · **657 JS + 153 Python tests**
...
| **D8** | **⚠️ OPEN — and it has now CAUSED an outage, not just a wrong number** | **Found 2026-08-22, not fixed, no user-facing impact.** `index.json`'s `extracts` block currently reads `games: 0, published: 0, absent: 4553`. That is not an outage: `catalog.json` still serves 4,553 games and 4,490 viewable, and every extract loads. **The nightly ingest runs `derive.py` (`ingest.yml:164`) against a store holding pointers and one night of raw**, so it writes a perfectly truthful report *of its own narrow run* over the archive-wide figures a full derive left there. `derive.yml` runs **weekly** (`cron: 20 9 * * 1`), so the ledger reads zeros for most of the week. **Nothing on the site reads that block** — the front door's freshness line uses `dataThrough`/`lastRun`/`coverage` — which is exactly why it went unnoticed. `builders/health.mjs` started reading it on 2026-08-21 and printed **"0 archived · 0 published"** at the top of this file the next morning; it now counts `catalog.json` instead, which is the document the site itself reads and cannot be emptied by a partial run. **⭐ AND ON 2026-08-23 IT BROKE THE NIGHTLY.** `unheldTypes`, shipped the day before, read the report's per-run `types` and so reported all eight named gameTypes as unheld on a run that walked nothing — failing the ingest with 4,553 games sitting untouched in the catalog. Fixed at the guard (both archive-claims now read the merged catalog, `a44594d`), but **the guard was only the second mistake.** The first is this row: a report whose figures describe a NARROW RUN while its name and its position say ARCHIVE is a trap laid for the next reader, and the next reader was a check written by the same person a day later. **That upgrades D8 from a cosmetic wrong number to a defect that has cost a production run.** **The report is still wrong.** Two candidate fixes, neither built: the nightly could MERGE rather than overwrite (keep the previous block when a run derived nothing), or `derive.py` could refuse to publish archive-wide figures from a run that saw a fraction of the archive — the second is closer to the doctrine, since a summary that describes a different population than its name implies is the C7 defect in a ledger. |
