...
|---|---|---|---|
| **C1** | **⭐ Discovery — a DATE path into the archive** | ⚠️ **THIS ENTRY'S OLD PREMISE WAS FALSE AND IS CORRECTED HERE.** It said *"three paths reach a game and 4,553 cannot be asked for."* Checked against the shipped code: `build_index.py` already ships a **team browse** (`?team=XXX&season=YYYY` — chips, season tabs, refused games listed with the check that stopped them), and **4,119 of 4,417 published games — 93.3% — are reachable today in three clicks.** The 298 that are not are preseason (287), all-star (3), Olympics (6) and 4 Nations (2), excluded everywhere by `inScope()` and disclosed on the front door. **So C1 is not "games cannot be reached", it is "there is only one way in"** — a smaller and truer claim that changes what gets built. Inherited and never re-derived; same failure mode as the blocked-shot flip. **Designed in `docs/discovery.md`, awaiting CHENG.** The shape: **calendar as the date index, list as the leaf, no toggle** — measured, because a team season is 40% full at **max 1 game per cell** (a calendar there is a worse list) while the league is 68% full at a median of 5 and a max of 16 (the cell carries a count). Four things want ruling: whether the 298 out-of-scope games appear (**50 dates are invisible on every current surface**), whether the calendar lives on the home page or its own (the home page already carries an index, and two indexes is the B2 attention problem, not a space problem — it physically fits at 2.9→3.3 screens on a phone), whether a date slice needs a base rate (CC says no — it selects on no outcome), and whether a cell counts games we HOLD (**69 of 667 nights contain a refusal and 2 are refused entirely**, so counting only what we can show prints `0` on a night hockey was played). | large |
