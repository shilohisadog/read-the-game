...
<sub>Generated 2026-08-23 by `node builders/health.mjs`. Archive figures COUNTED from the published `catalog.json` — the same document the site reads — as the pipeline last wrote it, whose own `lastRun` is 2026-08-22T11:22:03Z, covering games through 2026-06-14; the nightly moves them with no deploy, so re-run before quoting. Suite counts are checked by `npm run gates` and cannot go stale silently.</sub>
<!-- /health -->
