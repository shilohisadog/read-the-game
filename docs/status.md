...
**Health right now:** 4,553 archived · **4,490 published** · 63 refused · 73 unreconciled · 2,045 with a hero loop · **1,265 JS + 218 Python tests**
...
door features, and its reason has expired. ⚠️ **And a second one, same cause:**
the preview still turns the Attempts layer ON (`corsiOn=true;setCorsi()` in
`src/app.js`) so "the counts are built in front of you" — but the preview hides
the layer box and has no counters, so that choice now changes nothing a visitor
sees.
