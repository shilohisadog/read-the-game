...
**Health right now:** 4,561 archived · **4,497 published** · 64 refused · 73 unreconciled · 1,967 with a hero loop · **1,333 JS + 216 Python tests**
...
⏭ **OPEN, and Kevin's call:** the whistle layer draws its stacked-restart count with
the identical `m.n>1` rule, so the site now has **two conventions** — a zone-start ring
always shows its number and a whistle ring shows nothing when there is one restart.
Consistency argues for changing both; it is a two-character edit and was not made
unasked.
