...
**Health right now:** 4,553 archived · **4,490 published** · 63 refused · 73 unreconciled · **637 JS + 148 Python tests**

<sub>Generated 2026-08-21 by `node builders/health.mjs`. Archive figures from the published ledger, whose own `asOf` is 2026-08-21T15:07:34Z; the nightly moves them with no deploy, so re-run before quoting. Suite counts are checked by `npm run gates` and cannot go stale silently.</sub>
<!-- /health -->
