...
| **orchestration** | walks the store, judges, writes documents | `derive.py` | 790 |
| **analysis** | events → meaning; pure, no DOM, no network | `src/lib/**` (37 modules) | 7,622 |
| **measurement** | the archive, reduced by the SAME modules | `measure.mjs` | 649 |
| **presentation** | generates the pages | `build_*.py` (9) | 5,463 |
| **the app** | **the one exception — see §2** | `src/app.js` | 3,667 |

<sub>Counted 2026-09-11 by `tools/tiers.mjs`, checked by `npm run gates`. The analysis tier is **37 modules** and **not one of them touches the DOM, the network or the filesystem** — the boundary §1 claims, verified here rather than asserted. `src/app.js` **declares 28 dependencies on that tier and exports 1 function** — it is a module, not a build template, and §2 is what remains. Of its 3,667 lines **2,657 are comment-only and 883 are code**, and **226 comment lines carry an explicit claim** about the code beside them — which is §2's argument, counted rather than asserted.</sub>
<!-- /tiers -->
