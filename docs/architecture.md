...
| **analysis** | events → meaning; pure, no DOM, no network | `src/lib/**` (38 modules) | 7,741 |
| **measurement** | the archive, reduced by the SAME modules | `measure.mjs` | 649 |
| **presentation** | generates the pages | `build_*.py` (8) | 5,258 |
| **the app** | **the one exception — see §2** | `src/app.js` | 3,824 |

<sub>Counted 2026-09-20 by `tools/tiers.mjs`, checked by `npm run gates`. The analysis tier is **38 modules** and **not one of them touches the DOM, the network or the filesystem** — the boundary §1 claims, verified here rather than asserted. `src/app.js` **declares 29 dependencies on that tier and exports 1 function** — it is a module, not a build template, and §2 is what remains. Of its 3,824 lines **2,773 are comment-only and 922 are code**, and **249 comment lines carry an explicit claim** about the code beside them — which is §2's argument, counted rather than asserted.</sub>
<!-- /tiers -->
