...
| **analysis** | events → meaning; pure, no DOM, no network | `src/lib/**` (37 modules) | 7,673 |
| **measurement** | the archive, reduced by the SAME modules | `measure.mjs` | 649 |
| **presentation** | generates the pages | `build_*.py` (8) | 5,251 |
| **the app** | **the one exception — see §2** | `src/app.js` | 3,758 |

<sub>Counted 2026-09-18 by `tools/tiers.mjs`, checked by `npm run gates`. The analysis tier is **37 modules** and **not one of them touches the DOM, the network or the filesystem** — the boundary §1 claims, verified here rather than asserted. `src/app.js` **declares 28 dependencies on that tier and exports 1 function** — it is a module, not a build template, and §2 is what remains. Of its 3,758 lines **2,733 are comment-only and 900 are code**, and **242 comment lines carry an explicit claim** about the code beside them — which is §2's argument, counted rather than asserted.</sub>
<!-- /tiers -->
