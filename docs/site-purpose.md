...
| **B** | **Blocked shots** | `blk` is stored in every extract and surfaced nowhere. Audited in [`blocked-shots-layer.md`](blocked-shots-layer.md) and **CHENG has ruled**: no win rate at all (the blocks leader is the attempts trailer 81.7% of the time, so the 63.2% is *uninterpretable*, not merely uncertain), and the shipping sentence is a share of a population — *more than half of every team's attempts never reach the goalie*. He leans **inside control's "Show me the work" rather than a fifth toggle**: it is a correction to a number already on screen, not a new metric. | the reducer, the copy, and the archive figure through `measure.mjs`. **CHENG says land R first** — the blocked-shot mark is already on the ice, already misleading, and that is an R problem either way |
