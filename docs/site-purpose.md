...
| ~~**B**~~ | ~~**Blocked shots**~~ **DONE 2026-08-15** — live at `?layer=blocked`, its own toggle (Kevin's call over CC's and CHENG's: *Show me the work* is a verification surface, and this is the most novice-facing fact we have). Archive figure from `measure.mjs`: **491,971 attempts, 51.9% never reach the goalie, 27.8% blocked by a body**, shipped as a share of a population and never a win rate — the panel's own last line says why. The label names the BLOCKER, so the mark stops reading as the shooter's. | audit and rulings in [`blocked-shots-layer.md`](blocked-shots-layer.md) |
| ~~**R**~~ | ~~**The area below the rink is clunky**~~ **DONE 2026-08-16** — audit, CHENG's rulings and every measurement in [`below-the-rink.md`](below-the-rink.md). Read-once prose **576px → 122px in the first period**; the document **2.99 screens for everybody → 1.80 mid-game / 2.16 at the horn** for a returning viewer, 2.65 for a first-time one; the verdict card from screen 2.18 to **absent until the game ends**. Every sentence that stayed found a moment of use: the exclusion note fires when attempts drop out, the empty-net note while a net is empty (56px, 6% of frames), the ends key at the first period change, the amber tip with the layer that draws amber rings. **Nothing below the rink is now written for a reader who has already read it once**, except the legend naming the five marks the base view draws. | measured in a real browser at every step, never reasoned about |
...
| ~~**C**~~ | ~~**Cloudflare injects an analytics beacon into every browser request**~~ **DONE 2026-08-14** — Web Analytics was set to *"Enable, excluding visitor data in the EU"*, which still injects outside the EU; Kevin set it to **Disable**. Verified after: zero occurrences under a Chrome user-agent, and **zero console messages** on `/` and `/game` in a real browser, where there had been two and one. The gate's named exception is removed, so a future injection goes red. | and the hole it hid in is closed too: `fetch the live site back and diff it against the repo` now asks **as a browser**, because the injection was user-agent gated and that step had passed through all of it |
