...
test('the hero takes an older game to get a goal, and the kicker says which rule ran', () => {
  ...
  // `ha` WAS IN THIS SEED from 2026-09-11 to 2026-09-17, while the rule also read
  // an attempt-counter floor. That rule was dropped (the hero shows no counter),
  // so the loop alone decides again.
...
/**
 * ⏹ THE COUNTER FLOOR LEFT ON 2026-09-17, AT KEVIN'S WORD. The hero was chosen for
 * a loop whose attempt counter moved at least three times — and the hero has
 * shown no counter since 2026-09-09. Two tests pinned that rule; this one pins its
 * absence, so it cannot quietly return: a loop inside the window is a hero
 * whether or not the row carries an attempts figure, and whatever it says.
 * The window test above is the paired half — the loop still decides.
 */
test('the hero is chosen on the loop alone — an attempts figure changes nothing', () => {
  const seed = extra => ({ games: CATALOG.games.map(g =>
    g.id === 2023020100 ? { ...g, hl: 5, ...extra } : g) });
  return Promise.all([{}, { ha: 0 }, { ha: 2 }].map(extra => {
    const r = run({ docs: { ...ALL, 'catalog.json': seed(extra) } });
    return r.settle().then(() => {
      assert.equal(r.ids.herogo.href, 'game.html?game=2023020100',
        `a loop inside the window was rejected with ${JSON.stringify(extra)} — a counter rule is back`);
    });
  }));
});
