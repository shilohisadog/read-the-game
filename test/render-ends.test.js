...
/**
 * ⭐⭐ THE PAGE'S AWAY/HOME AXIS FOLLOWS THE ICE — Kevin, 2026-09-14: *"I think
 * the scoreboard should align with the ice."*
 *
 * ONE CLASS, NINE PAIRS. `drawSides` sets `#rg.hostleft` from `DIR(per)`, the
 * same function that places the nets, and nine surfaces read it in CSS.
 *
 * ⛔ THE CLASS BEING PRESENT PROVES NOTHING, which is why this is three
 * assertions and not one. A class that is never set passes "it agrees with the
 * ice" in every game where the host happens to open on the right; a class that
 * is always set passes it in every game where the host opens on the left; and a
 * class that flips on some other signal entirely passes "it changes" on its own.
 * So: it must AGREE with where the host's net actually is, on every frame; it
 * must CHANGE inside this game; and under the fixed-ends control it must not
 * change at all, because the rink does not.
 */
function walkSides(search) {
  const a = boot(null, null, search);
  const hAb = a.$('hAb').textContent;
  return a.every(d => {
    const tag = [...d.$('rink').innerHTML.matchAll(/<text class="netlab" x="([-\d.]+)"[^>]*>([^<]+)</g)]
      .find(m => m[2] === hAb);
    return { per: d.$('per').textContent,
             hostLeft: d.$('rg').classList.contains('hostleft'),
             netLeft: tag ? +tag[1] < 100 : null,
             aAb: d.$('aAb').textContent, hAb: d.$('hAb').textContent };
  });
}

test('the board turns over with the ice, and only with the ice', () => {
  const played = walkSides('?ends=as-played');

  let flips = 0;
  for (let k = 0; k < played.length; k++) {
    const f = played[k];
    // AGREEMENT, FRAME BY FRAME. `hostleft` claims the host's end is on the
    // screen-left; the host's NET is the thing that decides whether that is true.
    assert.equal(f.hostLeft, f.netLeft,
      `frame ${k} (${f.per}): the board says hostleft=${f.hostLeft} and the host's net is `
      + `on the ${f.netLeft ? 'left' : 'right'}`);
    if (k && f.hostLeft !== played[k - 1].hostLeft) flips++;
  }
  assert.ok(flips > 0,
    'the class never changed in a whole game, so agreement above proves only that it is stuck');

  // THE CONTROL. One-direction does not turn the rink over, so the board must not
  // turn over either -- and this is the half that fails if `drawSides` ever reads
  // the period instead of the transform.
  const fixed = walkSides('?ends=fixed');
  const distinct = new Set(fixed.map(f => f.hostLeft));
  assert.equal(distinct.size, 1,
    `the fixed-ends control flipped the board ${distinct.size} ways while the rink held still`);
});

test('⛔ the board is REPAINTED, never rewritten — no value follows the flip', () => {
  /* THE GUARANTEE THE WHOLE AS-PLAYED DESIGN RESTS ON, at a finer grain than
     "the ends toggle never reaches a count" one test below. That one proves the
     MODE reaches no reducer. This proves the per-period flip reaches no NODE:
     `#aAb` holds the away club and `#aSc` the away score in every frame of every
     period, whichever side they are painted on. If the flip were ever done by
     swapping content instead of by CSS `order`, this is what would catch it. */
  const played = walkSides('?ends=as-played');
  const [{ aAb, hAb }] = played;
  assert.notEqual(aAb, hAb, 'the fixture cannot tell the two apart');
  let sawLeft = 0, sawRight = 0;
  for (const f of played) {
    assert.equal(f.aAb, aAb, 'the away badge changed clubs when the ends turned over');
    assert.equal(f.hAb, hAb, 'the host badge changed clubs when the ends turned over');
    f.hostLeft ? sawLeft++ : sawRight++;
  }
  assert.ok(sawLeft > 0 && sawRight > 0,
    `the game never rendered both arrangements (${sawLeft} left, ${sawRight} right), `
    + 'so this proves nothing about the one it never showed');
});

/**
 * ⭐ THE LEDGER — every away/home pair on the page, and the rule that turns it.
 *
 * ⚠️ SIX OF THESE RENDER AND THREE ARE PARKED, and the split is measured rather
 * than read off the cascade: `.cbar`, `.counters` and `.pboxes` are
 * `display:none` under EVERY one of the seven layers. The first count said nine
 * live, because they were found hidden and a layer was ASSUMED to reveal them.
 * Their flips ship so a revival is not born mirrored; the parked assertion at
 * the foot of this test is what turns that assumption into a guard.
 *
 * ⛔ THE POINT IS THE COUNT. Kevin ruled the axis should follow the ice; the
 * defect to avoid is flipping SOME of it, which would put `CAR` on the left of
 * the board and `MTL` on the left of the box six inches beneath it — the same
 * disagreement, moved one surface over. Each entry names the pair and the
 * property that moves it, so a tenth pair added later has to be argued INTO this
 * list rather than quietly left behind.
 */
const AXIS = [
  ['.board .tm.h', /#rg\.hostleft \.board \.tm\.h\{order:1\}/, 'scoreboard badges, scores and penalty rows travel inside .tm'],
  ['.pboxes .pb.h', /#rg\.hostleft \.pboxes \.pb\.h\{order:1\}/, 'the penalty box'],
  ['.counters', /#rg\.hostleft \.counters,#rg\.hostleft \.cc\{flex-direction:row-reverse\}/, 'the attempt counters — and each half reverses too, because the markup hand-mirrors them'],
  ['.bar', /#rg\.hostleft \.bar,#rg\.hostleft \.pct\{flex-direction:row-reverse\}/, 'the control bar and its percentages'],
  ['.lxa', /#rg\.hostleft \.lxa,#rg\.hostleft \.lxan\{grid-column:3;text-align:right\}/, 'the layer output figures and names'],
  ['.lxh', /#rg\.hostleft \.lxh,#rg\.hostleft \.lxhn\{grid-column:1;text-align:left\}/, 'the layer output, other column'],
  ['narrow .tm.a', /#rg:not\(\.preview\)\.hostleft \.tm\.a\{grid-area:home\}/, 'the board\'s narrow form places by grid-area, which `order` cannot move'],
  /* ⛔⛔ THE ENTRY A SCREENSHOT ADDED, AND THE REASON THIS LIST IS NOT ENOUGH ON
     ITS OWN. `.tm.a` is [badge, score] and `.tm.h` is [score, badge] — a mirror
     so the badges sit outboard. Swapping which block is on the left without
     swapping the mirror inverts it, and BOTH arrangements are internally
     symmetric, so every assertion above still passed: the ledger was complete,
     `hostleft` agreed with the host's net on all 353 frames, and the rendered
     board was wrong. The pair enumerated was WHICH BLOCK IS ON THE LEFT; the
     defect was inside the block. */
  ['.tm.h mirror', /#rg:not\(\.preview\)\.hostleft \.tm\.h \.ab\{order:1\}/, 'the badge/score mirror inside the team block'],
  ['.tm.a mirror', /#rg:not\(\.preview\)\.hostleft \.tm\.a \.sc\{order:1\}/, 'the badge/score mirror, other side'],
];

test('every away/home pair on the page turns with the ice', () => {
  for (const [what, rule, why] of AXIS)
    assert.match(ENDS_CSS, rule, `${what} does not follow the ice — ${why}`);

  /* ⏸ AND THE THREE PARKED SURFACES ARE STILL PARKED. Their flips above were
     never seen rendering, so this is what stops that from being a silent
     assumption: the day one is revived, this fails and says go and look at it.
     A rule written for an invisible element is the one kind of claim in this
     file nobody can check by opening the page. */
  for (const [sel, rule] of [
    ['.cbar and .counters', /#rg \.cbar,#rg \.counters\{display:none\}/],
    ['.pboxes', /#rg \.pboxes\{display:none\}/]])
    assert.match(ENDS_CSS, rule,
      `${sel} is no longer parked — its hostleft flip has never been seen render; go and look at it`);

  // AND THE BASE ORDERS EXIST, because `order` on two of four grid children is
  // not a swap: `.foot` spans 1/-1 and auto-placement would push the board down
  // a row the moment a team block sorted ahead of it.
  assert.match(ENDS_CSS, /#rg \.board \.foot\{order:0\}/, 'the board caption lost its base order');
  assert.match(ENDS_CSS, /#rg \.pboxes \.pb\.a\{order:1\}/, 'the penalty box lost its base order');
});
