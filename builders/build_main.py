...
/* THE LABEL SAYS WHAT HAPPENED. A SECOND LINE IS ONLY EARNED BY SAYING WHETHER
   ...
   nothing.
   THE THREE THAT STAY ARE NOT DESCRIPTIONS, THEY ARE THE THESIS. A novice
   watching "Shot blocked" appear while the attempt counter GOES UP has just met
   ...
   than decorative. Same for a miss that still counts, and for a hit that looks
   like a shot in the run of play and is not one.
   The rule, so the next row does not have to be argued from scratch: a second
   line that rephrases the label is noise; a second line that says whether the
   event COUNTS is the work.
...
const LAB={faceoff:['Faceoff'],hit:['Hit','physical play — not a shot'],giveaway:['Giveaway'],takeaway:['Takeaway'],'blocked-shot':['Shot blocked','still an attempt — for the shooter'],'missed-shot':['Missed shot','wide/high — still an attempt'],'shot-on-goal':['Shot on goal'],penalty:['Penalty']};
let labelsOn=true;
/**
