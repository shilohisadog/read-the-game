...
#rg .mesh{stroke-width:.7;opacity:.85}#rg .post{stroke-width:1.1;stroke-linecap:round}
#rg .gkbody,#rg .gkhead{stroke-width:.55}#rg .gkstick{stroke-width:.7;stroke-linecap:round}
...
function drawRink(){const P=[];P.push('<rect class="boards" x="1" y="1" width="198" height="83" rx="27"/>');
 ...
 const netGlyph=(id,gx,col,fill)=>{
  ...
  return `<g class="netg">`
   ...
   + `<path class="mesh" d="${body}" fill="${fill}" stroke="${col}"/>`
...
const goalieGlyph=(gx,col,fill)=>{
 const dir=gx<100?1:-1, x=gx+2.6*dir;
 return `<g class="gk">`
  + `<rect class="gkbody" x="${(x-1.7).toFixed(1)}" y="40.4" width="3.4" height="5.2" rx="1.2" fill="${fill}" stroke="${col}"/>`
  + `<circle class="gkhead" cx="${x.toFixed(1)}" cy="39.3" r="1.25" fill="${fill}" stroke="${col}"/>`
  + `<line class="gkstick" x1="${(x+1.4*dir).toFixed(1)}" y1="43.6" x2="${(x+3.6*dir).toFixed(1)}" y2="45.8" stroke="${col}"/>`
  + `</g>`;};
let netmenAre=null;
