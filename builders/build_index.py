...
.scale .rl{font-size:.86rem;display:flex;justify-content:space-between;gap:12px;align-items:baseline}
.scale .rl b{font-weight:650}
...
.scale .axis{display:flex;justify-content:space-between;font-size:.72rem;color:var(--muted);
...
<main id="main">
  ...
  <div class="hero" id="hero" hidden>
    <p class="herokick">One game from the archive</p>
    <p class="heroline" id="heroline"></p>
    <p class="herosub" id="herosub"></p>
    <a class="herogo" id="herogo" href="game.html">Watch it happen &rarr;</a>
  </div>
</main>

<!-- THE LESSON FIRST, THE RATES AS EVIDENCE.
     Three equally weighted rows flattened the only interesting thing about
...
(function () {
  ...
  function drawHero(m, cat) {
    if (!m || !m.featured || !m.featured.length) return;
    var f = m.featured[0], g = null;
    for (var i = 0; i < cat.games.length; i++) {
      if (cat.games[i].id === f.id) { g = cat.games[i]; break; }
    }
    if (!g || !g.v) return;
    var lost = f.ab === g.h ? g.a : g.h;
    var ours = f.ab === g.h ? g.hs : g.as, theirs = f.ab === g.h ? g.as : g.hs;
    $('heroline').textContent = f.ab + ' controlled play and lost.';
    /* NO CAUSAL CONNECTIVE, and no adjective about how unusual it was. The
       numbers say the size; the sentence only says what happened. */
    $('herosub').textContent = f.ab + ' led the count of even-strength attempts taken '
      + 'while the score was level by ' + f.edge + ', and lost to ' + lost + ' '
      + theirs + '–' + ours + ' on ' + when(g.d) + '.';
    $('herogo').href = 'game.html?game=' + f.id;
    $('hero').hidden = false;
  }
