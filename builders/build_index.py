...
/* HIDDEN UNTIL THE SPACE EXISTS. `display:none` rather than a narrower column:
   below 1180 the hero card is a single column and there is no room beside the
   lede at all, so this would stack underneath and lengthen a phone screen that
   is already 4.5 screens tall. Kevin's call, with that number in front of him. */
.pucknote{display:none}
...
/* `:not([hidden])` for the same reason as `.opener .pucknote` and `.hero`: a bare
   `display:grid` outranks the UA sheet's `[hidden]`, so the empty grid a team page
   hides would stay in the flow. It has no children there, which is exactly why
   this one was invisible rather than obvious. */
.teams:not([hidden]){display:grid;grid-template-columns:repeat(auto-fill,minmax(74px,1fr));
...
/* ⭐⭐ THE FOLD, AT A LAPTOP'S WIDTH -- docs/front-door.md §5.4.
   ...
   visitor to click into the site."
   ...
   (front-door.md §5.1) -- it is not decoration, it is the slot.

   ⚠️ AND THE BAND HAS TO GROW OR THE RINK SHRINKS, which measuring is what
   showed. Two columns inside the 900px reading column would leave the frame
   ~520px against 898 today -- a 42% narrower rink to tidy the text beside it,
   which trades the product for the layout. So above 1180 the card alone breaks
   out of the column to `min(1240px, 100vw - 96px)`: at 1900 that is a 400px text
   column and an ~810px frame, 10% narrower than today rather than 42%.
   ⛔ THE REST OF THE PAGE STAYS AT 900. Prose has a measure; only the card has a
   picture in it.

   ⚠️⚠️ `:not([hidden])` IS LOAD-BEARING, NOT TIDINESS. `#hero` ships with the
   `hidden` attribute and `drawHero` clears it once a game is chosen, which works
   because nothing sets `display` on `.hero` -- the UA sheet's `[hidden]` wins by
   default. A bare `display:grid` here would OVERRIDE it and paint an empty white
   card on every page load before the catalog arrives, and on a page that never
   gets one it would stay. The state this protects is the one nobody screenshots.

   THE PHONE IS UNTOUCHED, on purpose: the whole query starts at 1180, and the
   novice test is on a phone (docs/site-purpose.md §9). */
@media (min-width:1180px){
  /* ⚠️ THE COLUMN WIDENS; THE CARD DOES NOT BREAK OUT OF IT. The first draft gave
     `.hero` a wider width and centred it with a translate, which put the card's
     left edge 148px outside the headline above it -- the eyebrow and the h1 stayed
     in the 900px column and the card jutted past them on both sides. LOOKING is
     what found it: every measurement of that draft was an improvement (the fold
     gained 313px) and it still read as a mistake.
     ⛔ AND IT IS `.wrap.front`, NOT `.wrap`. Four BODY templates in this file share
     this stylesheet -- the home page, learn, the rule pages and the calendar --
     and three of them are prose that has no business at 1240px. The home page's
     own BODY carries the class; nothing else can be widened by accident. */
  .wrap.front{max-width:1240px}
  /* ⭐ THE OPENING BLOCK BECOMES TWO COLUMNS, THE SAME MOVE THE CARD BELOW MAKES.
     ⛔⛔ IT IS `.opener` AND NOT `.lede`, AND THE REPO HAD ALREADY SAID SO. I
     named it `.lede`, and this stylesheet has carried `.lede{max-width:62ch}`
     all along — so the grid silently capped at 662px of a 1240px row, `1fr`
     collapsed to the note's longest WORD (32px), and the card was pushed 527px
     down the page. `build_main.py:54` warns in as many words: "NOT
     class=\"lede\" -- that name is spoken for", and `render-notes.test.js:1238`
     forbids it. The warning existed, in a comment, and I did not grep for it
     before taking the name. See the site-wide guard that now covers every page
     rather than only the one where the collision was first paid for.
     ⚠️ THE FIRST COLUMN IS WIDER THAN THE SENTENCE IN IT, AND THAT IS THE POINT.
     Borrowing the card's `minmax(320px,400px)` would squeeze the opening line
     from two rows to four — a layout change to the sentence a visitor judges the
     site by, paid for a note beside it. ⛔ AND `56ch` WAS NOT SAFE EITHER, WHICH
     ONLY MEASURING SHOWED: `ch` resolves against THIS element's font size and
     `.says` caps at 56ch of its OWN larger one, so the column came out 597.9px
     against the sentence's 603.5px cap — SIX PIXELS, and the lede wrapped to
     three lines. ⚠️ THE TWO-LINE LEDE THEREFORE HAS ABOUT 6px OF MARGIN and any
     type change flips it; the column is 62ch so the sentence keeps its own cap
     and nothing here decides its wrapping. The second column takes what is
     left. */
  .opener{display:grid;grid-template-columns:minmax(0,62ch) minmax(0,1fr);
   column-gap:clamp(20px,2.4vw,34px);align-items:end}
  .opener .eyebrow{grid-column:1;grid-row:1}
  .opener .says{grid-column:1;grid-row:2;margin-bottom:0}
  /* Bottom-aligned with the sentence beside it, so the two blocks share a
     baseline rather than both hanging from the top of an uneven grid. */
  /* ⚠️ `:not([hidden])` FOR THE REASON `.hero` CARRIES IT, ~250 lines up: "a bare
     display:grid here would OVERRIDE it". This was a bare `display:block`, so
     when a team page set `hidden` on this note the attribute was inert and the
     sentence stayed on screen — Kevin saw it there after the fix shipped. The
     warning was already written for the element next door. */
  .opener .pucknote:not([hidden]){display:block;grid-column:2;grid-row:1/span 2;align-self:end;
   margin:0;max-width:58ch;font-size:.87rem;line-height:1.5;color:var(--muted)}
  .opener .pucknote b{color:var(--ink);font-weight:650}
  /* ⛔ THE 78ch CAP IS GONE, AND IT WAS SOLVING THE WRONG HALF OF THE PROBLEM.
     It existed because the cards were full-width boxes in ONE column, so at
     1240 their text would have run to about a hundred characters a line. The
     cap fixed the measure and bought a 460px empty gutter beside every card --
     the only block on the front door that does not reach the edge the three
     above it reach. Kevin, looking at the laptop: "seems like we should make
     the 'what we claim' section multi column to align with everything else".
     TWO COLUMNS FIX BOTH: the measure comes from the COLUMN now, exactly as it
     does for `.cgrid` directly above, so there is nothing left for a cap to do.
     See the breakpoint beside `.limits` itself -- it is `.cgrid`'s, on purpose. */
  .hero:not([hidden]){
    display:grid;
    grid-template-columns:minmax(320px,400px) minmax(0,1fr);
    /* ⚠️ THE SIXTH ROW IS THE SLACK, and without it the text column is spread
       down the card. A grid item spanning five rows distributes ITS height across
       those rows, so the frame's ~470px was being handed to the five text rows --
       measured at 1900, the sentences sat 88, 83 and 119px apart, which reads as
       four unrelated notes rather than one thought. Five `min-content` rows plus
       one `1fr` underneath send the surplus to the bottom of the card instead. */
    grid-template-rows:repeat(5,min-content) 1fr;
    column-gap:clamp(20px,2.4vw,34px);
    align-items:start;
  }
  /* THE FRAME SPANS THE CARD'S WHOLE HEIGHT so the rink is as tall as the card,
     rather than as tall as whichever sentence happens to be longest. */
  .hero:not([hidden]) .heroframe{grid-column:2;grid-row:1/-1;margin:0}
  .hero:not([hidden]) .herokick{grid-column:1;grid-row:1}
  .hero:not([hidden]) .heroline{grid-column:1;grid-row:2}
  .hero:not([hidden]) .herosub{grid-column:1;grid-row:3}
  .hero:not([hidden]) .herorel{grid-column:1;grid-row:4}
  /* `justify-self:start` because a grid item stretches by default and the button
     is `inline-block`: without it the anchor's BOX fills the column while its
     background does not, so the click target and the thing that looks clickable
...
/* ⭐ THE SAME BREAKPOINT AS `.cgrid`, AND THAT IS THE POINT RATHER THAN A
   COINCIDENCE. These four cards sit two sections below the four measurement
   cards and are the same object -- a bordered box with a bold line and a
   paragraph under it -- so a width where one block is two columns and the other
   is one is a width where the page looks half-finished. A test pins the two
   numbers together so they cannot drift apart.
   TWO, NOT auto-fill: `.grid`'s `minmax(255px,1fr)` would give FOUR columns at
   1240 and set these sentences three words to a line. The limits carry the
   longest prose on the page, not the shortest. */
...
/* ⚠️ IT LANDS IN THE FIRST COLUMN, BECAUSE A NEAR-MISS READS AS A MISTAKE.
   This line sits directly under `.limits`, and the day that block became two
   columns its own 70ch cap (843px at 1400) stopped agreeing with anything on
   screen -- 228px wider than the card above it and 397 short of the block's
   right edge, which LOOKING showed as a stray fifth card hanging off the corner.
   Same breakpoint again, so above it the line is exactly one column and below it
   the grid is one column anyway and 70ch is the narrower cap. It is the only
   element on the site with this class, so the selector needs no page scope. */
/* 5.5px is HALF THE GRID GAP: `.limits` columns are (100% - 11px)/2, and this
   line shares their containing block, so the two right edges are identical
   rather than approximately so. */
@media (min-width:760px){.state{max-width:calc(50% - 5.5px)}}
...
<!-- ⭐⭐ THE ONE THING THE RINK DOES THAT A STRANGER READS AS BROKEN, SAID BEFORE
     THEY SEE IT. Kevin: "we don't mention we don't track puck movement, so the
     actual puck movement isn't shown in the replays... that's probably worth
     mentioning somewhere on the home page, since it's an integral part of what
     we can't show."
     ⛔ IT WAS SAID IN EXACTLY ONE PLACE ON THE WHOLE SITE, and that place is the
     game page's LEGEND: four words, `puck — jumps between real events`, on the
     page where the behaviour is already happening, reached after a visitor has
     been confused by it. The front door said nothing. `what-you-can-see.html` —
     fourteen learn cards and six rule pages — says nothing either.
     ⭐ A READING INSTRUCTION, NOT A CAVEAT. "The puck jumps because we do not
     have tracking data" is an apology that teaches a reader we are missing
     something; this says what the picture MEANS, which is the difference between
     a limit and a lesson.
     ⛔⛔ AND IT IS ABOUT OUR FEED, NOT ABOUT THE WORLD. The league DOES track the
     puck — it is simply not in the public play-by-play we read. "Nothing records
     where it went" would be false, and false in exactly the way this site exists
     to catch other people being.
     ⛔ ONE SENTENCE THIS MUST NEVER BECOME: "watch the marks, not the puck".
     Measured 2026-09-11 through three full loops — the rink holds ONE event at a
     time, on the front door and in the replay alike. There are no marks
     accumulating to watch, so that line would be false about the very rink
     underneath it.
     ⚠️ WIDE SCREENS ONLY, AND IT IS A TRADE KEVIN MADE WITH THE NUMBER IN FRONT
     OF HIM. Above 1180 this occupies 637x100 of space already being spent, to the
     right of a lede that caps at 56ch and directly above the rink — it costs
     nothing. Below 1180 the card is one column, the space does not exist, and
     this would add ~50px to a phone whose first screen he is separately judging
...
<!-- ⛔ WAS "Watch your team". The word implies streaming (Kevin), and unlike the
     ...
     to say otherwise. That is the rule the eleven uses of the word sorted into:
     it misleads only where nothing corrects it.
     AN INSTRUCTION RATHER THAN A PROMISE, which is also what the section is: the
     line under the grid already reads "Or browse by date", so the two ways into
...
# ⭐⭐ THE STYLESHEET THAT WAS NEVER WRITTEN. Kevin, 2026-09-23, off the live page:
# *"we need to do something with the data presentation, it's (honestly) ugly."* He
# was right and the cause was blunter than a design disagreement — the renderer
# emitted twelve class hooks and NOT ONE of them had a rule. The page was unstyled
# <p> tags in document order. There was no direction to disagree with.
#
# ⛔⛔ AND IT CARRIES A DOCTRINE AMENDMENT, so the next reader does not "fix" it
# back. `src/app.css` says the site has ONE way of showing a rate — a dot on a
# 0–100 track — and that sentence is TWO rules wearing one coat. The honesty half
# (never chart an empty population, every figure names its denominator, do not draw
# a gap as though it were a fact) is kept here in full. The house-style half is a
# declared choice, and Kevin re-declared it for this surface: *"a preview page
# isn't something we have had to render as of yet, can't our UX guidance evolve."*
# It can. The old rule's own justification — "the same idiom the homepage uses for
# the three base rates" — is already stale: the homepage renders its base rates as
# prose. One way of showing a rate has been one way ON THE GAME PAGE for a while.
PREVCSS = r"""<style>
.pvflag{color:var(--muted);font-size:.9rem;margin:6px 0 0}
.pvresult{margin:14px 0 0;font-size:1.05rem;font-weight:600}
.pvkick{font-size:.72rem;letter-spacing:.09em;text-transform:uppercase;
 color:var(--muted);font-weight:700;margin:34px 0 12px}
.pvn{font-size:.8rem;color:var(--muted);margin:10px 0 0;line-height:1.45}

/* ---- what is normal: three tiles ------------------------------------- */
.pvtiles{display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(215px,1fr))}
.pvtile{background:#fff;border:1px solid var(--edge);border-radius:11px;
 padding:14px 15px 15px;display:flex;flex-direction:column}
.pvbig{margin:0;font-size:2.1rem;line-height:1;font-weight:800;
...
/* ⛔ THE ROW IS DRAWN EVEN WITH NO CLUB FIGURE ON IT, and that is not a chart of
   ...
   season starts and only the two club bars are missing. Kevin asked to see the
   template; what makes it allowed is that the template is itself a measurement. */
.pvtrack{display:grid;grid-template-columns:auto 1fr auto;gap:5px 9px;
 align-items:center}
...
__HELPERS__
  ...
  /* ⭐⭐ THE TRACK, AND EVERY EDGE OF IT IS A MEASUREMENT.
     `range` is the min and max of the FULL-SEASON figures real clubs posted
     (`reliability.js` → measures.json), drawn as a shaded band, so the axis says
     "this is what clubs do" instead of spanning a number somebody picked. The
     axis itself stretches to hold whatever the two clubs are doing right now —
     which in October is routinely OUTSIDE the band, and seeing a club sit past
     the edge of every full season on record is the lesson, not a glitch.

     ⛔ FILL OPACITY IS `games / need`, OUTLINE ALWAYS SOLID. A bar drawn at full
     strength on twelve games looks exactly as authoritative as one drawn on
     sixty, and the whole point of the settle counts is that it is not. Kevin
     ruled the ramp; it is continuous, derived, and invents no cliff, which is
  ...
  function trackFor(row, sides) {
    ...
    if (!range && !vals.length) return null;

    var lo = range ? range.min : Math.min.apply(null, vals);
    var hi = range ? range.max : Math.max.apply(null, vals);
    vals.forEach(function (v) { lo = Math.min(lo, v); hi = Math.max(hi, v); });
    if (!(hi > lo)) return null;                 /* a zero-width axis draws nothing */
    var at = function (v) { return 100 * (v - lo) / (hi - lo); };
  ...
  function draw(p) {
    ...
    /* ⭐ MEASURE-FIRST, NOT CLUB-FIRST, AND THAT IS THE STRUCTURAL REPAIR. This
       ...
       forbade: the eye had to carry a number 200px down the page. One row per
       measure puts both clubs on the same axis. */
    var rows = p.clubs.away.rows;
    if (!rows.length) {
      sect.appendChild(el('p', 'pvnone', noneYet(p.counting)));
    }
    rows.forEach(function (ar, i) {
      var hr = p.clubs.home.rows[i];
      sect.appendChild(measureFor(ar, hr, p));
    });

    var links = el('p', 'pvlinks');
    ['away', 'home'].forEach(function (side) {
      var ab = p.clubs[side].ab;
      var link = el('a', null, 'Every ' + nameOf(ab) + ' game we hold →');
      link.href = '/?team=' + ab;
      links.appendChild(link);
    });
    sect.appendChild(links);
    $('pv').appendChild(sect);
  }
  ...
  function measureFor(ar, hr, p) {
    ...
    var sides = [ar, hr].map(function (r, i) {
      var c = i === 0 ? p.clubs.away : p.clubs.home;
      /* ⭐ HOW MUCH INK THE BAR HAS EARNED. Continuous in `games / need`, capped
         at 1 and never floored — the outline carries visibility, so no minimum
         had to be chosen. */
