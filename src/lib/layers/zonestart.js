...
export const zonestart = {
  ...
  reduce(events, ctx) {
    ...
    events.forEach((e, id) => {
      ...
      /* ⚠️ THE STRENGTH FILTER IS NOT DECORATION HERE, IT IS THE SECOND FINDING.
         `census.drawStrength` says a draw is worth 4.302x on the power play
         against 1.273x at even strength (ARCHIVE, n=22,790 / 190,144) — a BIGGER
         effect than the zone one. `one-measure.md` says one measure per screen,
         so this layer does not print that; but a reader who turns Even strength
         on is asking exactly the right question and must get an honest answer
         rather than a total that quietly mixes the two. */
