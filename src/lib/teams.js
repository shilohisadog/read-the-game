...
/**
 * ⭐ THE SIDES WE HAVE DELIBERATELY NOT NAMED — as a list the code can read.
 *
 * The paragraph above has always said which teams these are, and a paragraph is
 * not a check. `measure.mjs` walks every extract and reports any club `TEAMS`
 * cannot answer for, so that a relocation or an expansion team fails loudly
 * instead of arriving as a grey chip nobody notices. Fired against the real
 * archive it names CAN, FIN, HGS, KLS, KNG, MAT, MCD, MKN, SWE and USA — every
 * one of them a side this file has decided, on the record, not to describe.
 *
 * ⛔ A GUARD THAT FIRES ON EVERY RUN IS A GUARD SOMEBODY SWITCHES OFF. Its own
 * test says so — *"a check that fired on every run would be turned off within a
 * week, and then the expansion team would arrive invisibly"* — and until
 * 2026-09-09 it never got the chance, because `main()` threw a ReferenceError
 * two lines before the exit code that was supposed to be the alert.
 *
 * SO THE EXCEPTION IS NAMED, NOT INFERRED. Membership here is a decision with a
 * reason attached, and `test/teams.test.js` requires every entry to be a side
 * `TEAMS` really does not name — an entry that shadowed a real club would be a
 * hole in the check rather than an exception to it.
 */
...
export const NOT_A_CLUB = new Map([
  ...
  ['ITA', NATIONAL], ['FRA', NATIONAL], ['LAT', NATIONAL], ['DEN', NATIONAL],
  ['MCD', ALLSTAR], ['MAT', ALLSTAR], ['MKN', ALLSTAR], ['HGS', ALLSTAR],
  ['KNG', ALLSTAR], ['KLS', ALLSTAR], ['MUN', ALLSTAR],
]);
