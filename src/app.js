...
if(PREVIEW){
 ...
 /* ⭐ AND THE LOOP ENDS ON THE GOAL WHEN THERE IS ONE IN REACH.
    ...
    move START made at the other end: the opening frame is the game's first, and
    the closing frame is the first goal. Two ends, both the game's, and BUDGET_MS is left holding only the case where the archive
    hands us a game with no goal nearby.

    THE HERO IS CHOSEN SO THIS FIRES. builders/build_index.py picks the most
    recent game whose first goal is 3 to 8 plays after the opening faceoff. When it cannot (a small archive, a run
