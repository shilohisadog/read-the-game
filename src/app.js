...
/* ⭐ AND THE RINK'S PAINT SITS INSIDE IT, for the opposite reason.
   ...
   rinks. It is a real ES module -- builders/*.mjs imports it -- but it is NOT in
   LIB, because CHENG's ruling on as-played ends is that `SX` must be LEXICALLY
   unreachable from library scope, "not merely unused": a reducer that reads
...
let evenOnly=false;
/* The third rung. Separate from `evenOnly` because they are not the same
   claim: `tiedControl` bakes even strength in, so `levelOnly` implies it and
   deriving one from the other would make the ledger say it twice. */
let levelOnly=false;
...
/* The work overlay's name for the rung it is reporting. Three states now, and
   the third is not "even strength" with extra steps — it is a different
   population and has to say so, or the overlay narrates a count it is not. */
const MODE=()=>levelOnly?'score level, even strength, regulation'
                        :evenOnly?'even strength':'all situations';
...
/* ⭐⭐ THE COUNTING LADDER — one place, because three rungs in three call sites is
   three chances to disagree. Each rung is a STRICT SUBSET of the one above:

     all situations              corsi, everything the ice shows
     even strength only          corsi with `evenOnly` — drops power plays and
                                 the empty net
     while the score was level   `tiedControl`, which drops those AND an unlevel
                                 score AND overtime

   ⭐ THE THIRD RUNG NEEDED NO NEW REDUCER. `tiedControl` has existed and run on
   every game since the verdict card, excluding on the same five dimensions the
   work overlay already prints (`play`, `type`, `period`, `state`, `strength`) —
   so the exclusion ledger the strength design asked for is what this reducer has
   always produced. Wiring a control to it is the whole change.
   ⚠️ `tiedControl` takes no `evenOnly`: even strength is part of its DEFINITION,
   and making it optional would let a caller ask for a number with this one's
   name and not its meaning. */
function counted(slice){return levelOnly?tiedControl.reduce(slice,CTX)
                                        :corsi.reduce(slice,CTX);}
function lens(k){return counted(upto(k));}
...
/* THE PAINT IS IN src/lib/rinkart.js. `furniture()` is every fixed thing on an
   NHL sheet and carries no game in it; `netGlyph` is the only part that needs a
   period and a club colour, which is why it takes both rather than reading them.
   Kevin's diagrams on the learn page call the same `furniture()`, so the rink a
   novice is taught on and the rink they then watch cannot drift apart.
...
/* ⭐⭐ THE INTERRUPTED COUNTDOWN — B6, and it is a legibility defect rather than
   a missing feature.
   Kevin ruled the box static in August and the countdown answered him: name,
   infraction, clock. It teaches ITSELF when the clock runs out -- 0:07, then the
   seat is empty, and a novice joins those without help. IT TEACHES NOTHING WHEN
   IT DOES NOT. Read off the booted page on the reference game, the two endings
   side by side:

     #192 Kaprizov Interference 0:15      #84 Johnson Cross-checking 1:14
     #193 Kaprizov Interference 0:08      #85 Johnson Cross-checking 1:12
     #194 Kaprizov Interference 0:07      #86 (empty)
     #195 (empty)

   The right-hand column is a clock reading 1:12 and a man who vanished. That
   reads as a bug, and it is a RULE -- 16.2, a minor ends when the other team
   scores. Measured over 294 published games and 2,230 stints: 343 end on a goal,
   15.4%, about 1.2 a game, with a MEDIAN OF 62 SECONDS still showing.

   ⚠️ AND FIXING box.js MADE IT MORE COMMON, not less: bench minors and double
   minors now release correctly too, so the unexplained vanish happens more often
   than it did before 2026-09-07.

   ⛔⛔ IT APPEARS ON THE GOAL'S OWN FRAME AND NEVER ONE FRAME EARLIER, which is
   the whole difficulty. The comment above is emphatic that the clock counts the
   ASSESSED time because counting to the true end would LEAK THE FUTURE -- a
   clock reading 0:40 on a penalty a goal is about to kill tells the viewer a
   goal is coming. This note is the same hazard and the same answer: it is keyed
   to `cur.type === 'goal'`, so it can only ever appear on a frame where the
   reader has already been told a goal was scored. It explains; it never
   forecasts.

   THE ROW IS THE SAME TWO LINES TALL AS AN OCCUPIED SEAT, deliberately. The
   layout below the rink stopped moving on 2026-09-07 and a note that adds a
   third line for one frame would put the jitter back. So the reason REPLACES
   the infraction rather than joining it -- the infraction has been on screen for
   the whole penalty, and `Delay of game — illegal play by the goaltender` plus a
   clause is 66 characters against a two-line budget. */
function endedHere(tm,cur){
  if(!cur||cur.type!=='goal')return null;
  return PBOX.find(s=>s.team===tm&&s.endedBy==='goal'&&s.end===cur.s)||null;}
function drawBoxes(secs,cur){
 for(const [tm,id] of [[AID,'penA'],[HID,'penH']]){
   ...
   /* THE CLOCK IS STRUCK RATHER THAN BLANKED, because the unserved time is the
      fact worth keeping: he had 1:12 left and did not serve it. Two channels say
      it -- the strike for the eye and the sentence for the reader -- and the
      sentence carries the load, since a line through 0.72rem type on a phone is
      support, never the message.
      ⭐ THE SENTENCE NAMES THE SCORER, and that is a correction of a defect this
      project has already shipped once: a chip named the club opposite its own
      verb's subject. "The goal ended it" sits under a player's name and invites
...
function render(i,how){
 ...
 /* ⭐ EVERY INPUT NAMED, AND THERE ARE NINETEEN OF THEM. That number is the
    ...
    the trails mode. All of it was invisible while the loop sat here reaching for
    whatever it liked. Grouped into four so the list is a shape. */
 $('events').innerHTML=eventMarks(
  {evs,i,cur,moment},
...
function captionIcing(e){const z=ICING.get(e).zone;
 ...
 /* ⭐ THE CLUB GOES ON THE CHIP, NOT INTO THE PROSE. Kevin, 2026-09-18: *"unless
    we say which team … iced the puck"*. Every other caption names its club with
    the tag `sayCaption` already draws, and this one passed `null` because a
    stoppage had no club to name. It has one now (`offendingTeam`) — and putting
    it in the sentence instead would both cost width the measurement above says we
    do not have AND say the club twice, once on the chip and once in "back in
    BUF's end", which is the repetition the slot caption was already fixed for. */
 const who=ICING.get(e).offender;
 const side=who===AAB?'a':who===HAB?'h':null;
 sayCaption(side,who,'🧊 Icing',
  ' · from behind centre, past the far goal line'
  +(z?` &mdash; faceoff back in ${side?'their':ESC(z)+"'s"} end`:''),e);}
...
function captionOffside(e){
 /* ⭐⭐ AND WHEN THE DRAW CANNOT SAY, THE PILL SAYS THAT — in the rule's voice.
    5.60% of offsides across the archive restart at centre ice, which names no
    club (docs/stoppage-attribution.md). Two fallbacks for those were tested and
    both died; the better one agreed with the dot 54.38% of the time.
    ⛔ THE CLAUSE IS ABOUT THE ICE, NOT ABOUT OUR DATABASE. "Data not conclusive"
    describes OUR RECORD, and Kevin's own ruling on the penalties lede is that a
    sentence about what our record contains does not belong on a teaching
    surface: the test is whether it teaches the RULE. A centre-ice draw is a fact
    a reader can SEE — the dot is right there — and it is the same fact that
    tells them the draw's position is how the club is known at all.
    ⚠️ AND IT COSTS NOTHING THE PILL WAS NOT ALREADY PAYING, which is the test the
    icing caption above set. Measured on the built page against the rink beside it:
    at 844x390 — the landscape phone Kevin's ruling makes the surface — every
    caption on this pill is 3 lines and 53px, 25% of the ice, this one included.
    At 667x375 the clause takes it to 4 lines and 53% of the ice, which is exactly
    what icing's own sentence already costs there; at 1100 and above it is 2 lines
    and 9%. The clause never sets a new worst case. */
...
/* ⭐⭐ THE LENS SET, WRITTEN DOWN ONCE — and it was written down FOUR TIMES.
   ...
   as a URL token and "high-danger" is a term of art with definitions that are
...
/* ⚠️ AND ATTEMPTS GOES THROUGH `counted`, WHICH IS THE WHOLE POINT OF `counted`.
   The ladder was wired into `lens()` and the layer box first, and the chip under
   the scrubber kept its own way in through here — so pressing "while the score
   was level" moved the box and left the chip reading the all-situations total.
   Caught by the test that presses the control and reads the number a viewer
   sees, which a test calling the reducers would have missed entirely. This is
   also what puts the ladder's own exclusions into the work overlay, since
   `renderWork` reads `LEDGER` too: the exclusion ledger the strength design
   asked for is `tiedControl`'s `excluded`, printed by a panel that already
   knew how. */
const LEDGER=Object.fromEntries(Object.entries(LENS).map(([id,m])=>
...
function renderWork(_,cur,at){
 ...
 /* ⭐ EVERY INPUT NAMED, WHICH IS THE POINT OF THE MOVE. `src/lib/work.js`
    ...
    the one write. Two of these arguments are here because measuring found the
    cluster boundary was not where reading put it: `lboxFor` is SHARED with the
    layer box under the rink -- the two surfaces must agree -- and `cardsFor`
    cannot leave, because `LEARNCARDS` is injected by the builder and there is
...
/* ⭐ ALL THREE LABELS ARE SHORT, AND THAT IS A LAYOUT GUARANTEE AS WELL AS A COPY
   ...
   is 157px wide against `▶ Play` at 76, and at 390 that rewraps the button row --
   the transport went 156px to 210px on the pre-game frame, so the whole page
   below it jumped 54px at the exact moment a first-time visitor pressed the
   button. `▶ Replay` stays distinct at the horn because `Play` after the final
   whistle names the wrong action, and it is short enough not to rewrap. */
/* ⭐ ONE SOURCE FOR THE RESTING LABEL, because the first-visit greeting quotes it
   and `builders/build_main.py` ships it as the button's pre-script text. Three
...
/* ⭐⭐ THE POPUP THAT SAYS "you can check it" MUST APPLY THE RULE IT DESCRIBES.
   ...
   third added 2026-08-25 after Kevin looked at the drawn region and said "I
   don't consider the slot to be valid behind the net." The sentence below still
   named two and ended "Both true here", so the one surface on the site that
   hands a reader a ruler was handing them an incomplete rule: check a
   wrap-around against it and you get a different answer than our own ice does.
   Nothing mechanical could see this — the code was correct and the words were
   not, which is the only kind of defect this project actually has.

   ⭐ AND THE NUMBERS ARE NOW THE ONES THE RULE APPLIES. `dist` re-implemented
   `distanceToNet` and the thresholds were typed as literals, in a file that
   already resolves `attackDirection` out of the same module. A restated rule on
   a verification surface is one edit away from explaining a rule we do not use.

   ⛔ `angle < 22` BELOW IS NOT THIS 22. It is 22 DEGREES off straight-on and has
   nothing to do with `SLOT_HALF_WIDTH`, which is 22 FEET off centre. Two
...
/* ⛔⛔ `closest`, NOT `ev.target`, AND THIS WAS A LIVE DEFECT IN BOTH READERS.
   A click lands on the innermost element under the cursor. Most marks are a bare
   `<circle>` carrying `data-i`, so `ev.target.dataset.i` answered and everything
   looked fine for months. A mark drawn as a FIGURE is a `<g data-i>` with children
   -- and a goal is always drawn as a figure -- so the target is a `<path>` inside
   it and the index is undefined. Measured on the live site: clicking a goal's own
   mark with the slot layer on left `#whyBk` at `display:none` and `#whyContent`
   empty, while the page's caption says "Click a ring to see the distance and angle
   it was measured by."

   Kevin found it through the new highlight door -- "when I click on the goal event,
   the page still doesn't scroll down" -- and the older reader had the identical
   bug one line away. ⭐ *A fix that teaches one site about a variety and leaves its
   sibling* is a named failure here; both walk up now. */
...
function drawLabel(e){const g=$('labels');const p=place(e);if(!p){g.innerHTML='';return;}
 const lx=p.x,ly=p.y;
 /* ⭐ THE GOAL'S WORDS COME FROM `playSaid`, LIKE EVERY OTHER PLAY'S. This branch
    wrote its own sentence -- "🚨 GOAL — Stankoven" -- and it was the one label on
    the ice with no club: the club was left to the label's COLOUR. Kevin,
    2026-09-17: "we still don't say Goal - CAR - Goal Scorer". Two red clubs
    (batch 1's hero was Montreal at Carolina) and the colour answers nothing.
    Meanwhile `playSaid` already said "CAR · GOAL — …" for the share
    confirmation, so one moment was worded two ways. The order is the page's:
    TEAM · EVENT · PLAYER, the club as the subject of its play. What stays here is
    only what a goal adds -- the siren, the club colour, the assists line. And the
    name is ESCAPED now, which this branch alone never did.
    ⚠️ THE CLUB MADE THE LINE LONGER, SO IT WAS MEASURED, not assumed to fit: 330
    goals in the survivorship sample's 52 games, real Chromium, both viewports --
    0 overflow the rink, the tightest with 11 of 200 units to spare. The worst
    case that has NOT happened -- a short-handed goal by the longest name on the
    roster, at that tightest spot -- would run 9 units over. Kevin's fallback if it
    ever does: a smaller first line, never a smaller assists line. */
 if(e.type==='goal'){const tid=e.own,col=tid===AID?AWAYCOL:HOMECOL;
   const as=[R[e.a1],R[e.a2]].filter(Boolean).map(x=>x.nm).join(', ');
   let tx=lx>100?lx-5:lx+5,anc=lx>100?'end':'start',ty=Math.max(15,ly-6);
   g.innerHTML=`<g class="plabgrp"><line x1="${lx.toFixed(1)}" y1="${ly.toFixed(1)}" x2="${tx.toFixed(1)}" y2="${ty.toFixed(1)}" stroke="${col}" stroke-width=".4" opacity=".55"/><text class="glab" x="${tx.toFixed(1)}" y="${ty.toFixed(1)}" text-anchor="${anc}" fill="${col}">🚨 ${ESC(playSaid(e))}</text><text class="plabsub" x="${tx.toFixed(1)}" y="${(ty+4).toFixed(1)}" text-anchor="${anc}">${as?'assists: '+as:'unassisted'}</text></g>`;return;}
...
function playSaid(e){
 ...
 // The player is OPTIONAL: an unattributed goal ends at "CAR · GOAL" rather than
...
function drawZoneStarts(Z,slice){
 ...
 for(const d of dots.values()){
  ...
  /* ⭐ EVERY RING CARRIES ITS COUNT, INCLUDING 1 — Kevin, 2026-09-18: "we don't
     show the number 1 when there's only 1 draw on a face-off dot... I think we
     should show the number, even if it's 1."
     ⛔ WHAT THE OLD `d.n>1` WAS FOR, AND WHY IT DOES NOT SURVIVE. The count was
     added because the marks STACK: nine dots hold every draw in a game, so eight
     draws at one dot are one circle unless a number says otherwise. By that
     reasoning a lone draw needs no digit -- the ring already says one. But it
     makes ABSENCE carry meaning, and a reader then has to know the convention to
     read the ice. The only place it was ever stated is this layer's own help
     text, and "no number means one" is a rule about our drawing rather than
     anything about hockey. A number on every ring is read the same way by
     someone who was never told. */
  out.push(`<text class="zsn" x="${d.cx.toFixed(1)}" y="${d.cy.toFixed(1)}">${d.n}</text>`);
 }
...
function drawNewcomer(){
 ...
 /* ⭐⭐ AND THE ANSWER, WHICH THE ARCHIVE HAS ALWAYS HELD AND NO SURFACE SAID.
    `moreLevelControlLost` is computed by the same function over the same games
    and reached NOTHING until 2026-09-10 — so this page stated a paradox and left
    a novice to conclude that shot counts are meaningless, which is the opposite
    of the truth drawn from entirely true data. CHENG: *"the hook is not the
    mystery, it is that two honest counts of nearly the same thing land on
    opposite sides of 50%"* — the pair is the argument, and it is this site's
    thesis in two lines.
    ⛔ FRACTIONS, AND NOT FOR TIDINESS. The two rates have DIFFERENT
    denominators (a game with no level-play edge is not in the second), so two
    percentages side by side would hide the one thing a reader must see. */
 const R3=RATES&&RATES.baseRates&&RATES.baseRates.moreLevelControlLost;
 /* ⛔ THE CHIP NAMES ITSELF. This said `<b>Control</b>` — a layer that has not
    been called that since the picker was built, so the page told a first-time
    visitor to press something no longer on it. `chipLabel` is the one reader of
    that name and this is now its third caller; the comment on `pickLabel` eight
    lines above describes this exact failure — *"a second hand-typed copy is how
    the two start disagreeing after a rename"* — and the copy it warned about was
    already here. */
 const cn=chipLabel('corsi');
 const why=R2&&R2.n
   ?`<span class="nwhy">Across the whole archive, <b>the team with more shot `
    ...
    +`${R2.n.toLocaleString()} games.`
    +(R3&&R3.n
      ?` But count only the attempts taken <b>while the score was level</b>, and it `
       +`turns over: that team wins ${(R3.n-R3.count).toLocaleString()} of `
       +`${R3.n.toLocaleString()}. Which attempts you count changes the answer.`
      :'')
    +` <b>${ESC(cn)}</b> counts them on the ice, so you can `
 ...
 /* ⭐⭐ THE LABEL COMES FROM `PLAY_REST`, WHICH IS ALSO WHAT SETS THE BUTTON. The
    comment two hundred lines up already claims this sentence quotes the control
    "verbatim" — and it was a second copy, so when the button became `▶ Play` on
    2026-09-07 the greeting would have gone on naming a control no visitor could
    find. That comment RECORDS that happening once already, with `Add a metric
    layer`. A quotation with no link to its source is a copy waiting to drift.
    ⚠️ AND THE FIRST FIX READ `$('play').textContent`, WHICH IS THE WRONG SEAM.
    The fake document invents an element for any id and gives it an empty
    `textContent`, so the guard `$('play') ? … : …` passed and the greeting
    rendered `Press <b></b>` under test while a browser would have been fine —
    the fake-DOM absence trap in a new costume, caught only because the test asks
    the greeting to match the MARKUP's label rather than its own. A constant both
    sides read needs no DOM at all. */
 el.innerHTML=`<b>New here?</b> Press <b>${PLAY_REST}</b> and just watch — every event `
...
function drawLBox(k,L){
 ...
 const at=k==null?i:k, lens=L||(at<0?null:counted(upto(at)));
...
/* ⭐ THE CHIPS ARE BACK, AS A LADDER OF THREE — 2026-09-21, Kevin's ruling.
   They were removed on 2026-09-07 with a reason that was right at the time:
   "a novice cannot decide whether to press it without a paragraph about why
   power-play shots inflate a count, and that paragraph is exactly what the learn
   cards refuse to carry." The hold had a stated expiry and both halves of the
   paragraph have since shipped as cards a reader can open from this page —
   `All situations` for the strength half, `Score effects` for the score half.
   ⭐ Same grammar as trails and the ring: each button NAMES A STATE OF THE COUNT
   rather than a switch position, the summary carries the live setting because
   the drawer is closed, and the note describes what the OTHER choices would do. */
function syncStrength(){
 const v=levelOnly?'level':evenOnly?'even':'all';
 let lab='';
 document.querySelectorAll('#rg .sbtn').forEach(b=>{const on=b.dataset.s===v;
  b.setAttribute('aria-pressed',on);if(on)lab=b.textContent;});
 const z=$('zCountOn');if(z)z.textContent=lab.toLowerCase();
 const n=$('nCount');if(n)n.textContent=COUNT_NOTE[v]||'';
}
/* THE NOTE FOR EACH RUNG, and every one says what it REMOVES rather than what it
   keeps — "even strength only" tells a reader nothing unless they already know
   what else there was. The default branch describes the other choices, which is
   the 2026-08-16 rule: a button has to be predictable before the click. */
const COUNT_NOTE={
 all:'Every attempt in the game, which is what the marks on the ice add up to. '
    +'The other two take populations out and say which.',
 even:'Attempts taken while the sides were level in skaters. Power plays and an '
...
document.querySelectorAll('#rg .sbtn').forEach(b=>
 b.addEventListener('click',()=>setStrength(b.dataset.s)));
syncStrength();
...
/* ⭐ ONE CALL, NOT A SECOND SPELLING. This read `if(LINK.strength==='even')` and
   set the flag itself, which was harmless while there was one flag and would be
   a second implementation of the ladder now — `setStrength` is the one place the
   rung changes, and a deep link must move it the same way a click does. It is
   called before the first render, so it syncs rather than re-rendering. */
if(LINK.strength&&LINK.strength!=='all'){evenOnly=(LINK.strength==='even');
 levelOnly=(LINK.strength==='level');syncStrength();
 /* ⭐ AND THE DRAWER OPENS ITSELF, which is the second of the two safety halves
    every zone below the rink has to hold: "the zone OPENS ITSELF when it arrives
    with a layer on, so a deep link can always turn off what it turned on". A
    link that filters the count and leaves the control folded away is a page
    showing a number a reader cannot restore. The first half — the summary says
    what is on — is `zCountOn`, filled by `syncStrength` above. */
 const z=$('zCount');if(z)z.open=true;}
...
if(PREVIEW){
 ...
 /* ⏹ THE PREVIEW RAN WITH THE ATTEMPTS LAYER ON UNTIL 2026-09-17, so "the counts
    ...
    attempts reach the front door as a sentence, posted below. The loop opens at
    the faceoff -- see START below. */
 ...
 /* ⭐ THE LOOP BEGINS AT THE OPENING FACEOFF. Kevin, 2026-09-17: "start the
    hero at the opening faceoff -- that level sets everything and it's what a
    novice fan would expect."
    ⏹ Until then it opened ONE FRAME BEFORE THE FIRST COUNTED ATTEMPT, so the
    attempt counter read 0-0 and then moved in front of you. That reason expired
    with the counter (gone from the hero since 2026-09-09), and the rule it left
    behind skipped 1 to 10 opening plays in 33 of 54 archive games -- including
    today's hero, which opened on a hit. It also measured "first attempt" with
    the layer's reducer while derive.py used the event type, so the loop the
    front door chose and the loop that played could differ. From the faceoff
    both are the same count by construction: `hl` IS the first goal's index. */
 const START=0;
 let acc=0,W=START;
 while(W<EV.length-1&&acc+dwell(EV[W])<=BUDGET_MS){acc+=dwell(EV[W]);W++;}
 const BUDGETED=Math.max(START,W);
 /* ⭐ AND THE LOOP ENDS ON THE GOAL.
    Kevin: "let's end the hero replay right after the goal". The taste ends on
    the one event this renderer gives a real moment to, instead of wherever
    thirty seconds happened to run out. Build-up, payoff, restart.

    THE LENGTH IS THEREFORE THE DATA'S, NOT A NUMBER WE PICK: the opening frame
    is the game's first and the closing frame is its first goal. Two ends, both
    the game's. builders/build_index.py picks the most recent game whose first
    goal is 3 to 8 plays after the faceoff, then the newest with any first goal
    within derive.py's cap, and only then the newest game at all. */
 /* ⭐ AND IT ALWAYS ENDS ON THE GOAL, HOWEVER FAR THE GOAL IS. Kevin, 2026-09-17:
    "the hero always needs to end with a goal." The search was bounded by
    BUDGET_MS, so a hero whose goal is play 8 missed it whenever two frames
    before it carried a caption (4.5s against 3.6s: 8 frames can run 36s against
    a 30s budget) and the loop restarted on a hit. How long the loop runs is the
    SELECTOR'S job now (builders/build_index.py chooses a first goal 3 to 8 plays
    from the faceoff); the page's is only to stop on it. BUDGET_MS is left holding
    the one case with no goal to stop on -- a game whose goals, if any, all came
    in the shootout, which is not a place on the ice. */
 const GOAL=EV.findIndex((e,k)=>k>=START&&e.type==='goal'&&e.pt!=='SO');
